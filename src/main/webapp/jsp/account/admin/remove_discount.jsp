...
<fmt:message key="title.remove_discount" var="title"/>
...
<div class="container text-secondary">
    ...
    <c:choose>
        ...
        <c:when test="${not empty remove_discount_result}">
            ${remove_discount_result eq true? complete: failed}
        </c:when>
        <c:otherwise>
            <form method="post" action="${path}/controller" novalidate>
                <input type="hidden" name="command" value="remove_discount"/>
                <div class="row">
                    <label class="col-sm-2 col-form-label mb-3">
                            ${discount_id}
                    </label>
                    <div class="col-md-10">
                        <input type="text" value= "${discount_ses.entityId}" class="form-control" disabled>
                    </div>
                </div>
                <div class="row">
                    <label class="col-sm-2 col-form-label mb-3">
                    ...
                    <div class="col-md-10">
                        <input type="text" value= "${discount_ses.rate}" class="form-control" disabled>
                    </div>
